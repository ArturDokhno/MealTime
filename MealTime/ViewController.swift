...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let userName = "Artur"
        let feathRequest: NSFetchRequest<User> = User.fetchRequest()
        feathRequest.predicate = NSPredicate(format: "name = %@", userName)
        
        do {
            let results = try context.fetch(feathRequest)
            if results.isEmpty {
